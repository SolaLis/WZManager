...
import gjp.com.wzmanager.adapter.SqdrAdapter;
...
public class ASqbDrActivity extends BaseActivity implements ASqbmxContract.View {
    ...
    @BindView(R.id.tv_sqbmx_drck)
    TextView tvSqbmxDrck;
    @BindView(R.id.tv_sqbmx_dcck)
    TextView tvSqbmxDcck;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if ("1".equals(islistPush)) {
            setContentView(R.layout.activity_sqb_dr);
        }else {
            setContentView(R.layout.activity_sqb_dr_1);
        }
    ...
    private void init() {
        ...
        if ("006".equals(module)){
        headerTitle.setText("调入申请单");}
        else   if ("007".equals(module)){
            headerTitle.setText("调出申请单");}
    ...
    private void setHead() {
        ...
        tvSqbmxDrck.setText(sqbBean.getDrck());
        tvSqbmxDcck.setText(sqbBean.getDcck());
        tvSqbmxBz.setText(sqbBean.getBz());
    }
    private void setAdapter() {
            if ("1".equals(islistPush)){
                adapter = new SqdrAdapter(sqbmxBeanList,mHScrollViews,rvDrsq);
                //设置布局管理者
                LinearLayoutManager layoutManager=new LinearLayoutManager(ASqbDrActivity.this);
                layoutManager.setOrientation(LinearLayoutManager.HORIZONTAL);
                rvDrsq.setLayoutManager(layoutManager);}
            else {
                adapter = new SqdrAdapter(sqbmxBeanList, mHScrollViews1, rvDrsq, 1);
                //设置布局管理者
                LinearLayoutManager layoutManager = new LinearLayoutManager(ASqbDrActivity.this);
                layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
                rvDrsq.setLayoutManager(layoutManager);
            }
           // rvDrsq.setLayoutManager(new LinearLayoutManager(ASqbDrActivity.this));
            rvDrsq.addItemDecoration(new RecycleViewDivider(
                    ASqbDrActivity.this, LinearLayoutManager.HORIZONTAL, 1, R.color.color_b5));
            rvDrsq.setAdapter(adapter);
