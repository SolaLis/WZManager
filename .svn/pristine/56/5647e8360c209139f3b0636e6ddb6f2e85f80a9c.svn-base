...
import gjp.com.wzmanager.adapter.SqlyAdapter;
...
public class ASqbLyActivity extends BaseActivity implements ASqbmxContract.View {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if ("1".equals(islistPush)) {
            setContentView(R.layout.activity_sqb_ly);
        }else {
            setContentView(R.layout.activity_sqb_ly_1);
        }
    ...
    private void setAdapter() {
        if ("1".equals(islistPush)){
            //设置布局管理者
            adapter = new SqlyAdapter(sqbmxBeanList, mHScrollViews, rvLysq);
            LinearLayoutManager layoutManager = new LinearLayoutManager(ASqbLyActivity.this);
            layoutManager.setOrientation(LinearLayoutManager.HORIZONTAL);
            rvLysq.setLayoutManager(layoutManager);
        } else {
            adapter = new SqlyAdapter(sqbmxBeanList, mHScrollViews1, rvLysq, 1);
            //设置布局管理者
            LinearLayoutManager layoutManager = new LinearLayoutManager(ASqbLyActivity.this);
            layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
            rvLysq.setLayoutManager(layoutManager);
        }
            rvLysq.addItemDecoration(new RecycleViewDivider(
                    ASqbLyActivity.this, LinearLayoutManager.HORIZONTAL, 1, R.color.color_b5));
            rvLysq.setAdapter(adapter);
    }
