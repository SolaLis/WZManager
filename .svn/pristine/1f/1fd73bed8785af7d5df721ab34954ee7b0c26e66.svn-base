...
import gjp.com.wzmanager.adapter.SqcgAdapter;
...
public class SqbmxActivity extends BaseActivity implements SqbmxContract.View {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if ("1".equals(islistPush)) {
            setContentView(R.layout.activity_sqbmx);
        }else {
            setContentView(R.layout.activity_sqbmx_1);
        }
    ...
    public void getSqbmxSuccess(List<SqbmxBean> list) {
        ...
        sqbmxBeanList = list;
        Log.e("LOG", "getSqbmxSuccess(SqbmxActivity.java:44)===" + sqbmxBeanList.size());
    ...
    private void setAdapter() {
        if ("1".equals(islistPush)){
            adapter = new SqcgAdapter(sqbmxBeanList, mHScrollViews, rvCgsq);
            LinearLayoutManager layoutManager=new LinearLayoutManager(SqbmxActivity.this);
            layoutManager.setOrientation(LinearLayoutManager.HORIZONTAL);
            rvCgsq.setLayoutManager(layoutManager);}
        else {
            adapter = new SqcgAdapter(sqbmxBeanList,mHScrollViews1,rvCgsq,1);
            LinearLayoutManager layoutManager=new LinearLayoutManager(SqbmxActivity.this);
            layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
            rvCgsq.setLayoutManager(layoutManager);
        }
        rvCgsq.addItemDecoration(new RecycleViewDivider(
                SqbmxActivity.this, LinearLayoutManager.HORIZONTAL, 1, R.color.color_b5));
        rvCgsq.setAdapter(adapter);
    adapter.setOnItemLongClickListener(new MyItemLongClickListener() {
        @Override
        public void onItemLongClick(View view, int postion) {
            BaseUtil.myToast("111");
        }
    });
    }
    @Override
