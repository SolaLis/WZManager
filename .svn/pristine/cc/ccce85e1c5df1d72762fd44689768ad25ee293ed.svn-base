...
import gjp.com.wzmanager.adapter.PdmxAdapter;
...
import gjp.com.wzmanager.presenter.PdPresenter;
...
public class PdMxActivity extends BaseActivity implements PdContract.View {
    ...
    private PdContract.Presenter presenter;
    private String url;
    ...
    private List<PcMxBean> pcMxBeanList = new ArrayList<>();
    private AudirecordBean audirecordBean = new AudirecordBean();

    private PdmxAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if ("1".equals(islistPush)) {
            ...
            setContentView(R.layout.activity_pd_mx);
        }else {
            setContentView(R.layout.activity_pd_mx_1);
        }
        ...
        presenter = new PdPresenter(this);
        presenter.getPdmx(url, param);
    }
    ...
    @Override
    public void getPdmxSuccess(PdMxBean list) {
        ProgressDialogUtils.dismissProgressDialog();
        pcBean = list.getTmatser();
        pcMxBeanList = list.getListdata();
    ...
    private void setTotal() {
        for (int i = 0; i < pcMxBeanList.size(); i++) {
            totalNum = NumUtil.add(totalNum, pcMxBeanList.get(i).getSL());
            totalJs = NumUtil.add(totalJs, pcMxBeanList.get(i).getJS());
            totalPcNum = NumUtil.add(totalPcNum, pcMxBeanList.get(i).getPCSL());
            totalPcJs = NumUtil.add(totalPcJs, pcMxBeanList.get(i).getPCJS());
        }
    ...
    private void setAdapter() {
        if (adapter == null) {
            if ("1".equals(islistPush)){
            adapter = new PdmxAdapter(pcMxBeanList,mHScrollViews,rvPd);
            //设置布局管理者
            LinearLayoutManager layoutManager=new LinearLayoutManager(PdMxActivity.this);
            layoutManager.setOrientation(LinearLayoutManager.HORIZONTAL);
            rvPd.setLayoutManager(layoutManager);}
            else {
                adapter = new PdmxAdapter(pcMxBeanList,mHScrollViews1,rvPd,1);
                //设置布局管理者
                LinearLayoutManager layoutManager=new LinearLayoutManager(PdMxActivity.this);
                layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
                rvPd.setLayoutManager(layoutManager);
            }
          /*  adapter = new PdmxAdapter(pcMxBeanList);
            //设置布局管理者
            rvPd.setLayoutManager(new LinearLayoutManager(PdMxActivity.this));*/
            rvPd.addItemDecoration(new RecycleViewDivider(
                    PdMxActivity.this, LinearLayoutManager.HORIZONTAL, 1, R.color.color_b5));
    ...
    @Override
    public void getPdmxFailed(final String errorMessage) {
        ProgressDialogUtils.dismissProgressDialog();
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
